...
Color Color::mixed_with(Color const& other, float weight) const
{
    if (alpha() == other.alpha() || with_alpha(0) == other.with_alpha(0)) {
        return Gfx::Color {
            round_to<u8>(mix(red(), other.red(), weight)),
            round_to<u8>(mix(green(), other.green(), weight)),
            round_to<u8>(mix(blue(), other.blue(), weight)),
            round_to<u8>(mix(alpha(), other.alpha(), weight)),
        };
    }
    // Fallback to slower, but more visually pleasing premultiplied alpha mix.
    // This is needed for linear-gradient()s in LibWeb.
    auto mixed_alpha = mix(alpha(), other.alpha(), weight);
    auto premultiplied_mix_channel = [&](float channel, float other_channel, float weight) {
        return round_to<u8>(mix(channel * (alpha() / 255.0f), other_channel * (other.alpha() / 255.0f), weight) / (mixed_alpha / 255.0f));
    };
    return Gfx::Color {
