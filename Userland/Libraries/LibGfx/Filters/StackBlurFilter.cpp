...
void StackBlurFilter::process_rgba(size_t radius) {
  ...
  const auto stack_start = blur_stack.iterator_from_position(0);
  const auto stack_end = blur_stack.iterator_from_position(radius_plus_1);
  ...
  for (size_t y = 0; y < height; y++) {
    ...
    for (size_t x = 0; x < width; x++) {
      ...
      if (alpha_initial != 0) {
        set_pixel(x, y, Color(
          ((red_sum * mul_sum) >> shg_sum),
          ((green_sum * mul_sum) >> shg_sum),
          ((blue_sum * mul_sum) >> shg_sum),
          alpha_initial
        ));
      } else {
