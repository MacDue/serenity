...
void CanvasRenderingContext2D::stroke_rect(float x, float y, float width, float height)
{
    ...
    painter->stroke_path(path, drawing_state.stroke_style.to_color_fill_but_fixme_should_accept_any_fill_style(), drawing_state.line_width);
...
void CanvasRenderingContext2D::fill_text(DeprecatedString const& text, float x, float y, Optional<double> max_width)
{
    ...
    painter->draw_text(transformed_rect, text, Gfx::TextAlignment::TopLeft, drawing_state.fill_style.to_color_fill_but_fixme_should_accept_any_fill_style());
...
void CanvasRenderingContext2D::fill_internal(Gfx::Path& path, DeprecatedString const& fill_rule)
{
    ...
    painter->fill_path(path, *drawing_state().fill_style.to_gfx_fill_style(), winding);
    did_draw(path.bounding_box());
}
