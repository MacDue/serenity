...
ErrorOr<CanvasPathClipper> CanvasPathClipper::create(Gfx::Painter& painter, CanvasClip const& canvas_clip)
{
    ...
    RefPtr<Gfx::Bitmap> saved_bitmap;
    if (!maybe_bitmap.is_error() && !actual_save_rect.is_empty()) {
        saved_bitmap = maybe_bitmap.release_value();
    }
    painter.save();
    painter.add_clip_rect(bounding_box);
    return CanvasPathClipper(nullptr, {}, canvas_clip);
}
