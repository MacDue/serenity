...
ErrorOr<void> CanvasPathClipper::apply_clip(Gfx::Painter& painter)
{
    ...
    // Note: Doing the following (commented) translation fixes the black border on Google Street View:
    // actual_save_rect.translate_by(-1,-1);
    // However, it also makes the seams between quads/triangles very noticeable.
