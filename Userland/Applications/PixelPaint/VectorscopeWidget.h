...
constexpr size_t u_v_to_index(double u_v)
{
    double normalized_u_v = (u_v + 1.0) / 2.0;
    return static_cast<size_t>(floor(normalized_u_v * u_v_steps)) % u_v_steps;
}
constexpr double u_v_from_index(size_t index)
{
    double normalized_u_v = static_cast<double>(index) / u_v_steps;
    return normalized_u_v * 2.0 - 1.0;
}

struct PrimaryColorVector;

struct ColorVector {
    constexpr ColorVector(double u, double v)
        : u(u)
        , v(v)
    {
    }
    ...
    constexpr Gfx::FloatPoint to_vector(double scope_size) const
    {
        auto x = u * scope_size / 2.0;
        // Computer graphics y increases downwards, but mathematical y increases upwards.
        auto y = -v * scope_size / 2.0;
        return { x, y };
    }
...
struct PrimaryColorVector : public ColorVector {
    ...
    constexpr PrimaryColorVector(double u, double v, char symbol)
        : ColorVector(u, v)
        , symbol(symbol)
    {
    }
...
static constexpr double const pixel_percentage_for_max_brightness = 0.01;
...
private:
    ...
    Array<Array<double, u_v_steps + 1>, u_v_steps + 1> m_vectorscope_data;
