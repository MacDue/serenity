...
ErrorOr<void> HistogramWidget::rebuild_histogram_data()
{
    ...
    for (int i = 0; i < 256; i++) {
        m_data.red[i] = (static_cast<float>(m_data.red[i]) / max_color_frequency) * widget_height;
        m_data.green[i] = (static_cast<float>(m_data.green[i]) / max_color_frequency) * widget_height;
        m_data.blue[i] = (static_cast<float>(m_data.blue[i]) / max_color_frequency) * widget_height;
        m_data.brightness[i] = (static_cast<float>(m_data.brightness[i]) / max_brightness_frequency) * widget_height;
    }
