...
ErrorOr<int> serenity_main(Main::Arguments arguments)
{
    ...
    auto check_terminal_quit = [&]() -> int {
        bool has_foreground_process = tty_has_foreground_process();
        int child_process_count = 0;
        auto get_close_message = [&]() -> String {
            if (has_foreground_process) {
                return "There is still a process running in this terminal. Closing the terminal will kill it.";
            } else if (child_process_count > 1) {
                return String::formatted("There are background {} processes running in this terminal. Closing the terminal will kill them.", child_process_count);
            } else {
                VERIFY(child_process_count == 1);
                return "There is a background process running in this terminal. Closing the terminal kill it.";
            }
        };
        if (has_foreground_process || (child_process_count = shell_child_process_count()) > 0)
            return GUI::MessageBox::show(window, get_close_message(), "Close this terminal?", GUI::MessageBox::Type::Warning, GUI::MessageBox::InputType::OKCancel);
